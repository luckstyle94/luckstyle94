...
class GitHubAPI:
    ...
    def __init__(self, username: str, token: str = None):
        ...
        self.token = (
            token
            or os.environ.get("PROFILE_GH_TOKEN", "")
        ...
        self._token_login_checked = False
    ...
    def _fetch_stats_graphql(self) -> dict:
        ...
        repo_count, total_stars = self._aggregate_owned_repo_stats(include_private=True)
        total_commits = (
    ...
    def _fetch_stats_rest(self) -> dict:
        ...
        # Fetch repos to count stars (public only when no token)
        repo_count, total_stars = self._aggregate_owned_repo_stats(include_private=True)
        ...
        return {
            ...
            "prs": pr_count,
            "issues": issue_count,
            "repos": repo_count or user_data.get("public_repos", 0),
        }

    def _get_token_login(self):
        """Return token owner's login if available, else None."""
        if not self.token:
            return None
        if self._token_login_checked:
            return self._token_login

        self._token_login_checked = True
        try:
            resp = self._request("GET", f"{self.REST_URL}/user")
            if resp.status_code == 200:
                self._token_login = resp.json().get("login")
        except requests.exceptions.RequestException:
            self._token_login = None
        return self._token_login

    def _can_access_private_owned_repos(self) -> bool:
        """Whether the token can access this user's private owned repositories."""
        token_login = self._get_token_login()
        return bool(token_login and token_login.lower() == self.username.lower())

    def _paginate_repos(self, include_private: bool = True):
        """Yield pages of owned repos from REST API.

        If token belongs to target user and include_private=True, list with /user/repos
        so private repos are included. Otherwise, fallback to public /users/{username}/repos.
        """
        use_private = include_private and self._can_access_private_owned_repos()
    ...
    @staticmethod
    def _repo_log_label(repo: dict) -> str:
        """Return a safe repo label for logs without exposing private names."""
        if repo.get("private"):
            return "private-repo"
        return repo.get("full_name", "unknown")

    def _aggregate_owned_repo_stats(self, include_private: bool = True) -> tuple:
        """Return (repo_count, total_stars) across paginated owned repos."""
        repo_count = 0
        total_stars = 0
        for repos in self._paginate_repos(include_private=include_private):
    ...
    def fetch_languages(self) -> dict:
        ...
        languages = {}
        for repos in self._paginate_repos(include_private=True):
            for repo in repos:
                if repo.get("fork"):
                    continue
                try:
